...
docker run --rm \
    -v $PWD/config/secrets.json.template:/usr/src/app/secrets.json.template \
        ghcr.io/flare-labs-ltd/fasset-bots:main yarn key-gen generateSecrets \
    --agent $1 --other $2 --merge secrets.json.template -c "./packages/fasset-bots-core/run-config/${CHAIN}-bot.json" > secrets.new.json
